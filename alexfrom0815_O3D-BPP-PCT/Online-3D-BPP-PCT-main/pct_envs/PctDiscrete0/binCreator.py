...
class LoadBoxCreator(BoxCreator):
    def __init__(self, data_name=None):
        ...
        # print(len(self.box_trajs), "   ", self.box_trajs)
        print(len(self.box_trajs), "orders in box_trajs")
        # print(f"first order: {self.box_trajs[0]}")
        # print(f"last order: {self.box_trajs[-1]}")
    ...
    def reset(self, index=None):
        ...
        if index is None:
            ...
            self.index = index
        print(len(self.box_trajs[self.index]), "   ", self.box_trajs[self.index])
