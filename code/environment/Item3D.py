...
class Item3D():
    ...
    def getCoordinatesEdge(self, which:str) -> dict:
        ...
        if which == "north":
            coordinates = {
                "x": set(range(flbX, (flbX+length))),
                "y": set(range(flbY+width-1,flbY+width+1))
            }
        elif which == "east":
            coordinates = {
                "x": set(range(flbX+length-1, flbX+length+1)),
                "y": set(range(flbY, (flbY+width)))
            }
        elif which == "south":
            coordinates = {
                "x": set(range(flbX, (flbX+length))),
                "y": set(range(flbY-1, flbY+1))
            }
        elif which == "west":
            coordinates = {
                "x": set(range(flbX-1, flbX+1)),
                "y": set(range(flbY, (flbY+width)))
            }
    ...
    def __calculateDirectSupportSurface(self) -> None:
        if self.__ItemsBelow==[]:
            ...
            for itemBelow in self.__ItemsBelow:
                ...
                self.__PercentageDirectSupportSurface += (len(overlappingPointsX)*len(overlappingPointsY))/(self.__Representation.shape[0]*self.__Representation.shape[1])

                # create the DirectSupportSurface ndarray
                startX, startY = min(overlappingPointsX)-self.__FLBCoordinates[0], min(overlappingPointsY)-self.__FLBCoordinates[1]
                endX, endY = max(overlappingPointsX)-self.__FLBCoordinates[0]+1, max(overlappingPointsY)-self.__FLBCoordinates[1]+1

                self.__DirectSupportSurface[startY:endY, startX:endX] = np.ones((endY-startY, endX-startX), dtype=int)
    ...
    def __calculateEffectiveSupportSurface(self) -> None:
        if self.__ItemsBelow == []:
            ...
            for itemBelow in self.__ItemsBelow:
                ...
                self.__EffectiveSupportSurface[startY:endY, startX:endX] = np.ones((endY-startY, endX-startX), dtype=int)
            ...
            if len(self.__ItemsBelow) > 1:
                ...
                candMin1Extr2 = np.where(coordsEffectiveSupportSurface[:,0]==min1stCoord, coordsEffectiveSupportSurface[:,1], np.nan)
                min1stMax2nd = int(np.nanmax(candMin1Extr2))
                min1stMin2nd = int(np.nanmin(candMin1Extr2))

                candMax1Extr2 = np.where(coordsEffectiveSupportSurface[:,0]==max1stCoord, coordsEffectiveSupportSurface[:,1], np.nan)
                ...
                self.__EffectiveSupportSurface = np.multiply(self.__EffectiveSupportSurface, self.__DirectSupportSurface)
