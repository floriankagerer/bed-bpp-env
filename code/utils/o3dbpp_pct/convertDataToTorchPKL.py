...
def convertJSONDataToEvaluationPKL(src_order: pathlib.Path) -> None:
    ...
    for orderId, values in tqdm(benchmarkData.items()):
        ...
        for value in values["item_sequence"].values():
            ...
            if not (l in listLengths):
                listLengths.append(l)
            w = int(value.get("width/mm") / 10)
            if not (w in listWidths):
                listWidths.append(w)
            h = int(value.get("height/mm") / 10)
            if not (h in listHeights):
                listHeights.append(h)
...
if __name__ == "__main__":
    ...
    if not (isinstance(args.src_order, pathlib.Path)):
        args.src_order = pathlib.Path.cwd().joinpath(args.src_order)
