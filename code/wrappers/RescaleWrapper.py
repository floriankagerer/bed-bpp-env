...
class RescaleWrapper(gym.Wrapper):
    ...
    def __rescaleAllowedArea(self, allowedArea:dict) -> dict:
        ...
        for orientation, arrayAllowedArea in allowedArea.items():
            ...
            for _ in it:
                ...
                valRescaledIdx = np.amin(arrayAllowedArea[rescaledIdx[0]*self.__SIZE_DIVISOR[0]:(rescaledIdx[0]+1)*self.__SIZE_DIVISOR[0], rescaledIdx[1]*self.__SIZE_DIVISOR[1]: (rescaledIdx[1]+1)*self.__SIZE_DIVISOR[1] ])
    ...
    def __generateRescaledObservation(self, observation:np.ndarray) -> np.ndarray:
        ...
        for _ in it:
            rescaledIdx = it.multi_index
            valRescaledIdx = np.amax(observation[rescaledIdx[0]*self.__SIZE_DIVISOR[0]:(rescaledIdx[0]+1)*self.__SIZE_DIVISOR[0], rescaledIdx[1]*self.__SIZE_DIVISOR[1]: (rescaledIdx[1]+1)*self.__SIZE_DIVISOR[1] ])
            rescaledObs[rescaledIdx] = valRescaledIdx
