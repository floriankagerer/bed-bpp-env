...
static void displayAsBuiltMetricsWindow(void);
...
static void reshapeAsBuiltMetricsWindow(int width, int height);
...
static int asBuiltMetricsHeight = 495;   // height of as-built metrics window
static int asBuiltMetricsWidth = 275;    // width of as-built metrics window 
static int asBuiltMetricsWindow = 0;     // id of as-built metrics window
...
/* displayAsBuiltMetricsWindow

Returned Value: none

Called By: glInit (as glutDisplayFunc for asBuiltMetricsWindow)

This draws the asBuiltMetricsWindow.

*/

static void displayAsBuiltMetricsWindow(void) /* NO ARGUMENTS */
{
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  PalletViewer::printAsBuiltText(asBuiltMetricsHeight);
  glutSwapBuffers();
}
...
{
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  PalletViewer::printAsPlannedText(asPlannedMetricsHeight);
  glutSwapBuffers();
}
 ...
 const char * winName) /* name for graphics window               */
{
  ...
  for (i = 1; (int)i < argc; i+=2)
	...
	{
	  glutInitWindowPosition(asPlannedMetricsWidth + pictureExtent + 16, 0);
	  glutInitWindowSize(asBuiltMetricsWidth, asBuiltMetricsHeight);
	  asBuiltMetricsWindow = glutCreateWindow("As Built Metrics");
	  glutReshapeFunc(reshapeAsBuiltMetricsWindow);
	  glutDisplayFunc(displayAsBuiltMetricsWindow);
	  break;
	}
    }
 ...
 int y)
{
  switch (key)
    ...
    case 'f':
      ...
      glutPostRedisplay();
      if (asBuiltMetricsWindow)
	{
	  glutSetWindow(asBuiltMetricsWindow);
	  glutPostRedisplay();
	}
...
/* reshapeAsBuiltMetricsWindow

Returned Value: none

Called By: glInit (as glutReshapeFunc for the asBuiltMetricsWindow)

This reshapes the asBuiltMetricsWindow.

*/

static void reshapeAsBuiltMetricsWindow( /* ARGUMENTS                */
 ...
 int height)                             /* window height, in pixels */
{
  ...
  asBuiltMetricsHeight = height;
  asBuiltMetricsWidth = width;
}
...
asPlannedMetricsWindow, the pictureWindow, and the asBuiltWindow.
The asPlannedMetricsWindow is on the left side of the image, the
pictureWindow in the middle, and the asBuiltWindow on the right.
...
static int windowDump(void) /* NO ARGUMENTS */
{
  ...
  if (asBuiltMetricsWindow)
    {
      allWidth += (3 * asBuiltMetricsWidth);
      allHeight = max(allHeight, asBuiltMetricsHeight);
    }
  ...
  // handle the as-built metrics window
  if (asBuiltMetricsWindow)
    {
      glutSetWindow(asBuiltMetricsWindow);
      glPixelStorei(GL_PACK_ALIGNMENT, 1);
      glReadBuffer(GL_BACK);
      glReadPixels(0, 0, asBuiltMetricsWidth, asBuiltMetricsHeight,
		   GL_RGB, GL_UNSIGNED_BYTE, tempImage);
      start = (3 * (asPlannedMetricsWidth + pictureExtent));
      stop = (3 * asBuiltMetricsWidth);
      for (i = allHeight; i > asBuiltMetricsHeight; i--)
	{ // put black rectangle under the as-built metrics window if needed
	  for (j = 0;  j < stop; j++)
	    allImage[(i * allWidth) + start + j] = 0;
	}
      for (k=0; i >= 0; i--, k++)
	{ // copy in the as-built metrics window
	  for (j = 0;  j < stop; j++)
	    allImage[(i * allWidth) + start + j] = tempImage[(k * stop) + j];
	}
    }
